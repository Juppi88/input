	...
	case WM_CHAR:
		{
			...
			if ( ret ) input_handle_char_bind( (uint32)msg->wParam );
	...
	case WM_SYSKEYUP:
		{
			...
			if ( ret ) input_handle_key_up_bind( (uint32)msg->wParam );
	...
	case WM_SYSKEYDOWN:
		{
			...
			if ( ret ) input_handle_key_down_bind( (uint32)msg->wParam );
	...
	case WM_MOUSEMOVE:
		{
			...
			if ( ret )
				input_handle_mouse_move_bind( x, y );
	...
	case WM_MOUSEWHEEL:
		{
			return input_handle_mouse_event( INPUT_MOUSE_WHEEL,
				...
				(float)((short)HIWORD((DWORD)msg->wParam)) );
		}
	...
	case WM_LBUTTONDOWN:
		{
			...
			if ( ret )
				input_handle_mouse_down_bind( MOUSE_LBUTTON, x, y );
	...
	case WM_MBUTTONUP:
		{
			...
			if ( ret )
				input_handle_mouse_up_bind( MOUSE_MBUTTON, x, y );
	...
	case WM_MBUTTONDOWN:
		{
			...
			if ( ret )
				input_handle_mouse_down_bind( MOUSE_MBUTTON, x, y );
	...
	case WM_RBUTTONUP:
		{
			...
			if ( ret )
				input_handle_mouse_up_bind( MOUSE_RBUTTON, x, y );
	...
	case WM_RBUTTONDOWN:
		{
			...
			if ( ret )
				input_handle_mouse_down_bind( MOUSE_RBUTTON, x, y );
